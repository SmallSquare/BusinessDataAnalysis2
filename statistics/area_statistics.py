...
def get_area_statistics():
    province_list = []
    for province, city in get_areas().items():
        province_list.append(province)
    province_list.append("海外")
    province_list.append("其他")
    counts = {}

    for comment in database.get_comments():
        for province in province_list:
            if comment['area'] is None:
                continue
            if province in comment['area']:
...
def get_areas():
    area = {
        ...
        '宁夏': ['银川', '吴忠', '固原', '中卫', '石嘴山'],
        '西藏': ['拉萨', '日喀则'],
        '新疆': ['哈密', '和田', '喀什', '昌吉', '博乐', '伊宁', '塔城', '吐鲁番', '阿图什', '库尔勒', '五家渠', '阿克苏', '阿勒泰', '石河子', '阿拉尔', '乌鲁木齐',
               '克拉玛依', '图木舒克'],
...
if __name__ == '__main__':
    ...
    with open('as.csv', 'w', encoding='utf-8')as f:
        f_csv = csv.writer(f)
